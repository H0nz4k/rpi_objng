...
# (Ochrana pred prenosem z Windows bez LF normalizace.)
if [ -n "$(command -v bash 2>/dev/null)" ] && grep -qP '\r' "$0" 2>/dev/null; then
    sed -i 's/\r//' "$0"
    exec bash "$0" "$@"
fi
# Guard: ujistit se, ze bezime pod bash (ne sh/dash).
...
apt-get update
PACKAGES=(
  wget ca-certificates util-linux usbutils p7zip-full psmisc
...
# PackageKit drzi apt lock na Desktop RPi OS – vypnout pro spolehlivy firstboot apt.
systemctl disable --now packagekit.service 2>/dev/null || true
...
for f in \
  ...
  labwc-fullscreen.sh labwc_tk_helper.py restore-boot-terminal.sh teamviewer-resolve-alias.sh \
...
HOSTNAME="$(hostname -s 2>/dev/null || hostname 2>/dev/null || true)"
if [[ -n "$HOSTNAME" ]]; then
  if grep -qE '^127\.0\.1\.1[[:space:]]' /etc/hosts 2>/dev/null; then
    sed -i "s/^127\.0\.1\.1[[:space:]].*/127.0.1.1\t${HOSTNAME}/" /etc/hosts 2>/dev/null || true
...
rm -f "$TARGET_HOME/.local/state/objng-master-bootstrap/firstboot-install.lock"
