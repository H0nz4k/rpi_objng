...
Supported default profiles: 3M USB 0596:0001 and eGalaxTouch Virtual Device for Single.\nA default orientation preset may be applied before fine calibration.
"""
...
class CalibratorApp:
    def __init__(self, root: tk.Tk, dev: InputDevice, x_axis: AxisInfo, y_axis: AxisInfo) -> None:
        ...
        root.title("3M Touch Calibrator")
    ...
    def finish(self) -> None:
        ...
        color = "#29d17d" if rms <= 18 else "#ffb84c"
        ...
        self.canvas.create_text(
            self.width // 2, 342,
            text="Výsledek je uložený. V terminálu spusť:",
            fill="#a4b0bf", font=("Sans", 14)
        )
        command = f"sudo {apply_script}"
        self.canvas.create_text(
            self.width // 2, 385, text=command,
            fill="#29d17d", font=("Monospace", 16, "bold")
        )
        self.canvas.create_text(
            self.width // 2, 460,
            text="Potom restartuj zařízení:   sudo reboot\n\n"
                 "Poznámka: vytvořený apply skript před změnou zálohuje původní udev pravidlo.",
            fill="#d5deeb", font=("Sans", 14), justify="center"
        )
        self.canvas.create_text(
            self.width // 2, self.height - 45,
            text="Esc = zavřít kalibrátor",
            fill="#8a98aa", font=("Sans", 12)
        )
